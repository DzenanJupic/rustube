...
pub mod signature_cipher {
    use serde::{Deserialize, Deserializer};
    use serde::de::{Error, Unexpected};
    use url::Url;

    use crate::player_response::streaming_data::SignatureCipher;

    pub fn deserialize<'de, D>(deserializer: D) -> ::std::result::Result<SignatureCipher, D::Error>
        where
